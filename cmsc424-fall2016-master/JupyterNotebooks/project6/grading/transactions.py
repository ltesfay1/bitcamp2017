...
class LockTable:
	...
	compatibility_list = [(IS, IS), (IS, S), (S, IS), (S, S), (IX, IX), (IX, IS), (IS, IX)]
	@staticmethod
	...
	@staticmethod
	def dfs(vertex, htable, path, visited, result):
		if vertex in visited:
			return []
		visited.add(vertex)
		path.add(vertex)
		for next in htable.get(vertex, None):
			if next not in result:
				if next in path or LockTable.dfs(next, htable, path, visited, result):
					return vertex
		path.remove(vertex)
		return []
	...
	# 	def visit(vertex):
	# 		path.add(vertex)
	# 		for neigh in htable.get(vertex, ()):
	# 			if neigh in path or visit(neigh):
	# 				return vertex
	# 		path.remove(vertex)
	# 		# return []
	# 	return result.append(visit(v) for v in htable)
	# @staticmethod
	# def visit(vertex, path, htable):

	# 	path.add(vertex)
	# 	for neigh in htable.get(vertex, ()):
	# 		if neigh not in result:
	# 			if neigh in path or LockTable.visit(neigh, path, htable):
	# 				return vertex
	# 			path.remove(vertex)
	...
	def detectDeadlocksAndChooseTransactionsToAbort():
		...
		result = []
		with LockTable.hashtable_lock:
			for key, value in (LockTable.lockhashtable).iteritems():
				holdlist = value.current_transactions_and_locks
				waitlist = value.waiting_transactions_and_locks
				for x in waitlist:
					for y in holdlist:
						if x[0] in htable:
							htable[x[0]].append(y[0])
						else:
							htable[x[0]] = [y[0]]
			
			for vertex in htable:
				path = set()
				visited = set()
				temp = LockTable.dfs(vertex, htable, path, visited, result)
...
class LogManager:
	...
	def restartRecovery():
		...
		for item in allrecords:
			if item:
				if item.info[1] == LogRecord.UPDATE:
					if Relation.getRelationByName(item.info[2]):
						tup = Relation.getRelationByName(item.info[2]).getTuple(item.info[3])
					if tup:
						tup.setAttribute(item.info[4], item.info[6])
				elif item.info[1] == LogRecord.START and item.info[0] not in undolist:
					undolist.append(item.info[0])
				elif item.info[1] == LogRecord.ABORT or item.info[1] == LogRecord.COMMIT and item.info[0] in undolist:
					undolist.remove(item.info[0])

		for item in allrecords:
			if item and item.info[1] == LogRecord.START and item.info[0] in undolist:
				LogManager.revertChanges(item.info[0])
				LogManager.createAbortLogRecord(item.info[0])
				undolist.remove(item.info[0])
