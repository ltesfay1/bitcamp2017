...
class LockTable:
        ...
        with cond:
            while True:
                ...
                if not e.current_transactions_and_locks:
                    print "Able to get this lock on " + objectid
                    ...
                    if not e.waiting_transactions_and_locks:
                        ...
                        if compatible:
                            print "Compatible so able to get this lock on " + objectid
                            e.current_transactions_and_locks.append((transaction_id, locktype))
                            return True
                        else:
                            print "Unable to get this lock on " + objectid + " so waiting"
                            e.waiting_transactions_and_locks.append((transaction_id, locktype))
                    else:
                        print "Someone else is waiting for a lock on " + objectid + " so waiting"
                        e.waiting_transactions_and_locks.append((transaction_id, locktype))
    ...
    @staticmethod
    def dfs(vertex, htable, original, path, visited, killed):
        if vertex in visited:
            return []
        visited.add(vertex)
        path.add(vertex)
        for next in htable[vertex]:
	    if next not in killed:
                if next in path or LockTable.dfs(next, htable, original, path, visited, killed):
                    return original
        path.remove(vertex)
        return []        
    ...
    def detectDeadlocksAndChooseTransactionsToAbort():
        ...
        with LockTable.hashtable_lock:
            for entry in LockTable.lockhashtable:
                holdlist = LockTable.lockhashtable[entry].current_transactions_and_locks
                waitlist = LockTable.lockhashtable[entry].waiting_transactions_and_locks
                for x in waitlist:
                    for y in holdlist:
                        if x[1] in htable:
            ...
            for vertex in htable:
                path = set()
                visited = set()
                temp = LockTable.dfs(vertex, htable, vertex, path, visited, result)
                if temp and temp not in result:
                	for entry in LockTable.lockhashtable:
                		holdlist = LockTable.lockhashtable[entry].current_transactions_and_locks
                		for x in holdlist:
                			if x[0] == temp and x[1] not in result:
                				result.append(x[1])
                    # result.append(temp)
        return result    
...
class LogManager:
	...
	for lr in reversed(allrecords):
	    if (lr.info[1] == LogRecord.ABORT or lr.info[1] == LogRecord.COMMIT):
	        if (lr.info[0] not in li):
		    li.append(lr.info[0])

	undorecords = [lr for lr in reversed(allrecords) if lr.info[0] not in li]
	for lr in undorecords:
		...
		x = LogRecord([lr.info[0], LogRecord.ABORT])
		LogManager.writeLogRecord(x)
